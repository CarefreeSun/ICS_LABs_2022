...
volatile sig_atomic_t waitflag; /* parent process wait for fg child to end */
...
eval(char *cmdline) 
{
    ...
    if (tok.builtins == BUILTIN_QUIT) /* quit */
        exit(0);
    else if (tok.builtins == BUILTIN_JOBS) {/* jobs */
        ...
        return;
            ...
            || tok.builtins == BUILTIN_FG) { /* bg & fg*/
        sigprocmask(SIG_BLOCK, &mask_all, &oldest);
        ...
        sigprocmask(SIG_SETMASK, &oldest, NULL);
        if(pid > 0){
            if (tok.builtins == BUILTIN_BG){ /* bg */
                printf("[%d] (%d) %s", jid, pid, job->cmdline);
                kill(-abs(pid), SIGCONT);
                job->state = BG;
            }
            else { /* fg */
                kill(-abs(pid), SIGCONT);
    ...
    else if (tok.builtins == BUILTIN_KILL) { /* kill */
        sigprocmask(SIG_BLOCK, &mask_all, &oldest);
        if (tok.argv[1][0] == '%'){ /* jid */
            jid = atoi(tok.argv[1] + 1);
            job = getjobjid(job_list, abs(jid));
    ...
    /* builtin cmd end */
        ...
        if ((pid = fork()) == 0){ /* child process */
            ...
            /* in/out file path */
            if(tok.infile){
                int input_fd = open(tok.infile, O_RDONLY, 0);
                dup2(input_fd, STDIN_FILENO); 
            }
            if(tok.outfile){
                int output_fd = open(tok.outfile, O_WRONLY, 0);
                dup2(output_fd, STDOUT_FILENO);
            }
            char * exec_path;
            if(tok.builtins == BUILTIN_NOHUP) exec_path = tok.argv[1];
            else exec_path = tok.argv[0]; /* deal with nohup */
            if(execve(exec_path, tok.argv, environ) < 0){
...
sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED | WCONTINUED)) > 0){
        ...
        if(WIFEXITED(status)){ /* end normally */
        ...
        else if(WIFSIGNALED(status)){ /* end due to signal */
            if(tmp->state == FG){
                waitflag = 0;
            }
            sio_put("Job [%d] (%d) terminated by signal %d\n", 
                tmp->jid, tmp->pid, WTERMSIG(status));
            deletejob(job_list, pid);
        }
            ...
            sio_put("Job [%d] (%d) stopped by signal %d\n", 
                tmp->jid, tmp->pid, WSTOPSIG(status));
            tmp->state = ST; /* change from FG to ST */
        }
